...
export default function Configuracoes({ onLogout }: { onLogout: () => void }) {
  ...
  // Mock states to simulate sync and data completeness from PC
  const [isSynced] = useState(true);
  const [hasLogo] = useState(true);
  const [hasCompleteData] = useState(true);
  ...
  const [empresa] = useState({
    nome: 'Cantina Modelo',
    tipo: 'Cantina Escolar / Empresarial',
    telefone: '+244 900 000 000',
    nif: '5000000000',
    email: 'contacto@cantinamodelo.co.ao'
  });

  // Location State (Read-only in mobile)
  const [localizacao] = useState({
    pais: 'Angola',
    provincia: 'Luanda',
    municipio: 'Luanda',
    bairro: 'Mutamba, Rua da Missão'
  });

  // Security State (Read-only in mobile)
  const [seguranca] = useState({
    login: 'admin',
    funcao: 'Administrador',
    telefoneRecuperacao: '+244 900 000 000'
  });
  ...
  return (
    <div className="p-4 space-y-6 pb-24 max-w-2xl mx-auto">
      ...
      <div className={`space-y-6 transition-opacity duration-300 ${!isUnlocked ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        {/* Dados da Empresa */}
        <section className="bg-theme-card glass-blur rounded-3xl p-6 shadow-sm border border-theme-border space-y-4 relative overflow-hidden">
        ...
        <section className="bg-theme-card glass-blur rounded-3xl p-6 shadow-sm border border-theme-border space-y-6 relative overflow-hidden">
