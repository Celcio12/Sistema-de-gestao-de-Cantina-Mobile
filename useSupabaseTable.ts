...
const DIRECT_TABLES = ['products', 'employees', 'sales', 'settings', 'clients', 'notifications'];
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          // Filtro de synckey apenas se não for directo (pois tabelas directas não têm a coluna)
          filter: isDirect ? undefined : `synckey=eq.${SYNC_KEY}`,
        },
        (payload) => {
          if (!isMounted) return;
